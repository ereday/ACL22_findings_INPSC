...
def main_loop(model,optimizer,schedular,train_dataloader,eval_dataloader,args,logger,ix2label,clusters):
    ...
    for i_ in tqdm(range(int(args['max_num_train_epochs'])), desc="Epoch"):
        ...
        for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
            ...
            loss = model(input_ids, segment_ids, input_mask, label_ids,clusters=clusters)
            loss.backward()
            tr_loss += loss.item()
...
def run_fold_experiment(params,trn_df,val_df,tst_df,is_final=False):
    ...
    # Clusters are not used in this code. It is exist because the file was copied from bert_main_regularizer_interdistance.py
    clusters = []
...
def main_final():
    ...
    params = {                
                'output_dir':'./outputs/base_models',
               'output_model_file':'./outputs/base_models',
               'only_generate': False,
